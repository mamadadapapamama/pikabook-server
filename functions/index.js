...
function getGeminiClient() {
  const apiKey = process.env.GEMINI_API_KEY || process.env.OPENAI_API_KEY; // 테스트를 위해 우선 호환 가능하게 설정
...
- No trailing commas`;
    ...
    console.log(`🚀 Gemini API 호출 시작 (모델: gemini-2.0-flash, 모드: ${mode})`);
    ...
    console.log(
        `📄 Gemini 응답 내용 (처음 200자): "${content.substring(0, 200)}..."`,
    );

    try {
      // Gemini는 JSON 모드 설정 시 순수 JSON만 반환하므로 추가 정리 불필요
